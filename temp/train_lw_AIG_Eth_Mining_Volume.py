...
def load_args():
    ...
    parser.add_argument('--batch-size', type=int, default=32,
                        help='batch size')
    parser.add_argument('--act_func', type=str, help='activation function', default='elu',
                        choices=ACT_FUNC.keys())
    parser.add_argument('--trans_act_func', type=str, help='transformer activation function', default='gelu',
                        choices=['relu', 'gelu'])
    ...
    for t in range(args.training_times):
        ...
        for k in range(args.k_ford):
            ...
            split_idx = dataset.get_idx_split(X=np.arange(len(dataset)),
                                              ...
                                              seed=seeds[t], K=args.k_ford, k_idx=k)
            ...
            for epoch in range(args.epochs):
                ...
                if args.early_stop:
                    ...
                    f1_list.append(test_score)
            print('Result in terms of f1-score: {} ~ {}\n\n\n'.format(np.mean(f1_list), np.std(f1_list)))
            all_score_list.append(f1_list)
            if args.early_stop:
                k_fold_test_score_list.append(f1_list[0])
