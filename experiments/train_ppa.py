...
def load_args():
    ...
    parser.add_argument('--use-edge-attr', action='store_true', help='use edge features')               #True
    parser.add_argument('--edge-dim', type=int, default=128, help='edge features hidden dim')
    parser.add_argument('--gnn-type', type=str, default='graph',                                        #Base GNN model
                        choices=GNN_TYPES,
                        help="GNN structure extractor type")
    parser.add_argument('--k-hop', type=int, default=2, help="number of layers for GNNs")
    parser.add_argument('--global-pool', type=str, default='mean', choices=['mean', 'cls', 'add'],      #Aggregate node-level representations into a graph representation.
...
def main():
    ...
    if args.not_extract_node_feature:
        ...
        transform = partial(extract_node_feature, reduce=args.aggr)                      #***提取节点初始特征，聚合操作默认为相加***
    ...
    # print(train_dset[0])
    # print(train_dset[0].degree)
    # print(len(train_dset[0].degree))
